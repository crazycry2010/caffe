...
void read_image(std::ifstream* file, int* coarse_lable, int* label, char* buffer) {
  char label_char;
  file->read(&label_char, 1);
  *coarse_lable = label_char;
  file->read(&label_char, 1);
  *label = label_char;
...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    ...
    CHECK(data_file) << "Unable to open train file train.bin";
    for (int itemid = 0; itemid < 5*kCIFARBatchSize; ++itemid) {
      read_image(&data_file, &coarse_label, &label, str_buffer);
...
int main(int argc, char** argv) {
  if (argc != 4) {
    printf("This script converts the CIFAR dataset to the leveldb format used\n"
           "by caffe to perform classification.\n"
           "Usage:\n"
           "    convert_cifar100_data input_folder output_folder db_type\n"
